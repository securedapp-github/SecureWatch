...
function Alerts() {
  ...
  const { monitorName, email, m_id, token, network, address, rk, selectedEventNames, category } = location.state || {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Monitor Updated successfully!", {
        autoClose: 500,
        onClose: () => {
          navigate("/monitor", { state: { email, token, riskCategory,actionType } });
        },
      });
    } catch (error) {
