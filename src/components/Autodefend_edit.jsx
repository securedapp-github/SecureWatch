...
function Autodefend_edit() {
  ...
  const { abi, name, email, m_id, network, address, rk, alert_data, alert_type, slack_webhook } = location.state || {};

  const [Abi, setAbi] = useState(JSON.parse(abi) || []);
  const [events, setEvents] = useState([]);
  const [existingDefenders, setExistingDefenders] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch events
        const eventsResponse = await fetch("https://139-59-5-56.nip.io:3443/get_event", {
          method: 'POST',
          headers: {
        ...
        console.log("Events Data:", eventsData.monitors);
        
        setEvents(eventsData.monitors || []);

        // Fetch existing defenders
        const defendersResponse = await fetch("https://139-59-5-56.nip.io:3443/getDefender", {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (!defendersResponse.ok) throw new Error('Failed to fetch existing defenders');
        const defendersData = await defendersResponse.json();
        console.log("Existing Defenders Data:", defendersData.data);
        setExistingDefenders(defendersData.data || []);

        // Initialize selected events and functions from existing defenders
        const initialSelectedEvents = [];
        const initialEventFunctions = {};

        defendersData.data.forEach(defender => {
          initialSelectedEvents.push(defender.eid);
          
          // Find the function in ABI
          const func = Abi.find(f => f.name === defender.name && f.type === "function");
          if (func) {
            initialEventFunctions[defender.eid] = {
              function: func,
              params: defender.params || {}
            };
          }
        });

        setSelectedEvents(initialSelectedEvents);
        setEventFunctions(initialEventFunctions);
  ...
  const handleFunctionSelect = (eventId, selectedOption) => {
    ...
    setEventFunctions(prev => ({
      ...
      [eventId]: {
        ...
        params: prev[eventId]?.params || {} // Preserve existing params if any
      }
    }));
  };
                ...
                {selectedEvents.includes(event.id) && (
                  <div className="mt-4 p-4 border rounded">
                    <h2 className="text-lg font-semibold mb-2 text-black">
                      {event.name} - Select Function
                    </h2>
                    <Select
                      options={Abi
                        ?.filter(func => func.type === "function")
                        .map(func => ({ value: func.name, label: func.name }))}
                      onChange={(option) => handleFunctionSelect(event.id, option)}
                      placeholder="Select Function"
                      className="mb-4 text-black"
                      isMulti={false}
                      value={eventFunctions[event.id]?.function ? { 
                        value: eventFunctions[event.id].function.name, 
                        label: eventFunctions[event.id].function.name 
                      } : null}
                    />

                    {eventFunctions[event.id]?.function && (
                      <div className="mb-6 p-4 border rounded">
                        <h2 className="text-md font-semibold mb-2 text-black">
                          {eventFunctions[event.id].function.name} Parameters
                        </h2>
                        {eventFunctions[event.id].function.inputs?.map(input => (
                          <div key={input.name} className="mb-2">
                            ...
                            <input
                              type="text"
                              value={eventFunctions[event.id]?.params?.[input.name] || ""}
                              onChange={(e) => handleFunctionInputChange(e, event.id, input.name)}
          ...
          <div className="mt-4 md:mt-0 border mx-auto md:mx-0 border-[#2D5C8F] shadow-md p-5 rounded-xl">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              {events.length > 0 ? (
                ...
                <div className="text-[13px]">No events available</div>
              )}
            </div>
            <div className="mt-3">
              <div className="font-medium w-40 flex justify-between items-center" style={{ color: "black" }}>
